...
def load_system_config(system_config=get_system_config()):
    ...
    if system_config is None:
        logger.warning("system_config.yaml is empty or could not be loaded, returning None")
        raise ValueError("System config is None")

    if not isinstance(system_config, dict):
        logger.error("system_config.yaml does not contain a valid dictionary, returning None")
        raise ValueError("System config is not a dictionary")

    if "system_prompt" not in system_config or not isinstance(system_config["system_prompt"], str):
        logger.warning("system_prompt key missing or not a string in system_config, returning None")
...
def load_config(config_path):
            ...
            "input": {
                ...
                "properties": {
                    "resource_data": {
                        "type": "object",
                        "oneOf": [
                            {"properties": {"files": {"type": "array", "items": {"type": "string"}}}, "required": ["files"], "not": {"required": ["folder"]}},
                            {"properties": {"folder": {"type": "string"}}, "required": ["folder"], "not": {"required": ["files"]}}
                        ]
                    },
                    "modules": {
                        "type": "object",
                    ...
                    "failback": {
                        ...
                        "allOf": [
                            {"if": {"properties": {"type": {"const": "modules"}}}, "then": {"required": ["options"]}},
                            {"if": {"properties": {"type": {"const": "resource"}}}, "then": {"not": {"required": ["options"]}}}
                        ]
                    }
                },
    ...
    if "bedrock" in config and "output_json" in config["bedrock"]:
        if isinstance(config["bedrock"]["output_json"], str):
            try:
                config["bedrock"]["output_json"] = json.loads(config["bedrock"]["output_json"])
