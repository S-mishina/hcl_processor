...
from .output_writer import validate_output_json,output_md
...
def run_hcl_file_workflow(file_path, config, system_config):
    ...
    try:
        ...
        validated_output = validate_output_json(output_str, config["bedrock"]["output_json"])
        ...
        if config["input"]["failback"]["enabled"] == True:
            module_name = get_modules_name(resource_dict)
            if config["input"]["failback"]["type"] == "resource":
                # TODO: Not yet guaranteed to work
                resources = resource_dict["resource"][0]
            else:
                resources = resource_dict["module"][0][module_name][config["input"]["failback"]["options"]["target"]]
            try:
                for resource in resources:
                    combined_str = f"{locals_str}\n{resource}\n"
                    partial_output = aws_bedrock(combined_str, modules_raw, config, system_config)
                    validated_partial = validate_output_json(partial_output, config["bedrock"]["output_json"])
            ...
            for json_obj in hcl_output:
                try:
                    ...
                    logger.debug(f"{e}")
                    logger.error(f"Error extending flattened list with: {json_obj}, error: {type(e).__name__}")
