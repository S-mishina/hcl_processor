...
from .config_loader import load_system_config, load_config
        ...
        if resource.get("files"):
            ...
            for file_path in resource["files"]:
                try:
                    ...
                    logger.debug(f"{e}")
                    logger.exception(f"Failed processing file {file_path}: {type(e).__name__}")
                    continue
            ...
            for root, _, files in os.walk(resource["folder"]):
                for file_name in files:
                    if file_name.endswith(".tf"):
                        try:
                            run_hcl_file_workflow(os.path.join(root, file_name), config, system_config)
                        except Exception as e:
                            logger.debug(f"{e}")
                            logger.exception(f"Failed processing file {file_name}: {type(e).__name__}")
                            continue
