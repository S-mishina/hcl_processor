...
def build_system_config():
    return {
        ...
        "default_bedrock": {
            "timeout_config": {"read_timeout": 10, "connect_timeout": 10, "retries": {"max_attempts": 3, "mode": "standard"}},
            "payload": {"anthropic_version": "v1", "max_tokens": 100, "temperature": 0.5, "top_p": 1.0, "top_k": 40}
        }
    }
...
def test_aws_bedrock_success(mock_session):
    ...
    mock_streaming_body.read.return_value = json.dumps({"content": [{"text": "response text"}]}).encode("utf-8")
