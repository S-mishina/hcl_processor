...
def build_config():
    return {
        ...
        "output": {"markdown_path": "/tmp/test_output.md"},
    }
...
def test_bedrock_provider_invoke_single_success(mock_session):
    ...
    assert (
        call_kwargs["toolConfig"]["tools"][0]["toolSpec"]["inputSchema"]["json"]
        == config["provider_config"]["settings"]["output_json"]
    )

    expected = json.dumps(
        {"monitors": [{"monitor_name": "Test Monitor", "type": "query alert"}]},
        ensure_ascii=False,
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ClientError,
            {"Error": {"Code": "Test", "Message": "normal client error"}},
            ClientError,
        ),
        (
            ClientError,
            {
                "Error": {
                    ...
                    "Message": "Input token size exceeds limit",
                }
            },
...
def test_bedrock_provider_api_exceptions(
    mock_session, exception_type, exception_message, expected_exception
):
    mock_client = MagicMock()

    if exception_type == ClientError:
        exception_instance = ClientError(
            error_response=exception_message, operation_name="test"
        )
        ...
        exception_instance = ReadTimeoutError(
            endpoint_url="test", error=exception_message
        )
    else:
...
def test_bedrock_provider_json_decode_error(mock_session):
    mock_client = MagicMock()
    mock_response = {
        "output": {"message": {"role": "assistant", "content": [{"invalid": "format"}]}}
    }
    mock_client.converse.return_value = mock_response
    mock_session.return_value.client.return_value = mock_client
...
def test_bedrock_provider_defaults_used(mock_session):
    ...
    config = build_config()
    config["provider_config"]["settings"]["payload"].pop(
        "max_tokens", None
    )  # force default use
    system_config = build_system_config()
    provider = BedrockProvider(config, system_config)
...
def test_bedrock_provider_array_schema_wrapped_in_object(mock_session):
    ...
    call_kwargs = mock_client.converse.call_args.kwargs
    sent_schema = call_kwargs["toolConfig"]["tools"][0]["toolSpec"]["inputSchema"][
        "json"
    ]
    ...
    expected = json.dumps(
        [{"monitor_name": "Test Monitor 1"}, {"monitor_name": "Test Monitor 2"}],
        ensure_ascii=False,
    )
    assert result == expected
