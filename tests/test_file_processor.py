...
import os
from src.hcl_analytics.file_processor import run_hcl_file_workflow, read_tf_file, read_local_files, get_modules_name
...
@patch("src.hcl_analytics.file_processor.hcl2.loads", return_value={"key": "value"})
...
@patch("src.hcl_analytics.file_processor.hcl2.loads", side_effect=Exception("parse error"))
...
@patch("src.hcl_analytics.file_processor.open", new_callable=mock_open)
@patch("src.hcl_analytics.file_processor.os.makedirs")
@patch("src.hcl_analytics.file_processor.aws_bedrock", return_value='{"key": "value"}')
@patch("src.hcl_analytics.file_processor.hcl2.loads", return_value={"resource": [{}], "module": [{"module_name": {"target": [{}]}}]})
def test_run_hcl_file_workflow_success(mock_hcl2, mock_bedrock, mock_makedirs, mock_open_func, tmp_path):
    ...
    system_config = {}
    with patch("src.hcl_analytics.file_processor.validate_output_json", return_value={"validated": True}), \
         patch("src.hcl_analytics.file_processor.output_md") as mock_output_md:
        run_hcl_file_workflow(str(file_path), config, system_config)
        mock_output_md.assert_called()

@patch("src.hcl_analytics.file_processor.read_tf_file", return_value=(None, None))
...
@patch("src.hcl_analytics.file_processor.aws_bedrock", side_effect=json.decoder.JSONDecodeError("Expecting value", "", 0))
@patch("src.hcl_analytics.file_processor.get_modules_name", return_value="module_name")
@patch("src.hcl_analytics.file_processor.validate_output_json", return_value=[{"result": True}])
@patch("src.hcl_analytics.file_processor.read_tf_file", side_effect=[("content", "dir"), ("content", "dir")])
@patch("src.hcl_analytics.file_processor.read_local_files", return_value="locals")
@patch("src.hcl_analytics.file_processor.hcl2.loads", return_value={"resource": [{"r1": {}}], "module": [{"module_name": {"target": [{}]}}]})
@patch("src.hcl_analytics.file_processor.open", new_callable=mock_open)
@patch("src.hcl_analytics.file_processor.os.makedirs")
def test_run_hcl_file_workflow_failback(mock_makedirs, mock_open_func, mock_hcl2, mock_read_local, mock_read_tf, mock_validate, mock_get_module, mock_bedrock, tmp_path):
    ...
    system_config = {}
    with patch("src.hcl_analytics.file_processor.output_md") as mock_output_md:
        run_hcl_file_workflow(str(file_path), config, system_config)
        mock_output_md.assert_called()
